...
async def main():
    ...
    for x_val, theta_val in zip(X_test_values, theta):
        Y_prediction += x_val * theta_val

    print("\nY_Prediction value for the full test data:")
    ...
    for x_val, theta_val in zip(descaled_test_data, descaled_theta):
        Y_prediction_scaled += x_val * theta_val

    print("\n**Scaled Values check** Y_Prediction value for the full test data (w/scaled values), result expected to be identical:")
    print(f"{Y_prediction_scaled}, classifying as {'M' if round(Y_prediction) == 1 else 'B'}")
    
    # Perform element-wise multiplication and summation on subset_sm data
    Y_prediction_subset_sm = 0
    for x_val, theta_val in zip(X_test_values, theta_subset_sm):
        Y_prediction_subset_sm += x_val * theta_val
        
    print("\nY_Prediction value for the subset_sm test data:")
    print(f"{Y_prediction_subset_sm}, classifying as {'M' if round(Y_prediction) == 1 else 'B'}")
    
    # Perform element-wise multiplication and summation on subset_lg data
    Y_prediction_subset_lg = 0
    for x_val, theta_val in zip(X_test_values, theta_subset_lg):
        Y_prediction_subset_lg += x_val * theta_val
        
    print("\nY_Prediction value for the subset_lg test data:")
    print(f"{Y_prediction_subset_lg}, classifying as {'M' if round(Y_prediction) == 1 else 'B'}")
